            ...
            <div class="row">
                ...
                <div class="col-lg-5">
                    <div class="ltn__checkout-payment-method mt-50">
                        ...
                        <button type="submit" class="btn theme-btn-1 btn-effect-1 text-uppercase mt-3">Đặt hàng</button>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('/api/ghn/provinces')
        ...
        .then(data => {
            ...
            if (data.status || data.success) {
                ...
                data.data.forEach(p => {
                    province.innerHTML += `<option value="${p.ProvinceName}" data-id="${p.ProvinceID}">${p.ProvinceName}</option>`;
                });
            } else {
    ...
    province.addEventListener('change', function() {
        ...
        .then(data => {
            ...
            if (data.status || data.success) {
                data.data.forEach(d => {
                    district.innerHTML += `<option value="${d.DistrictName}" data-id="${d.DistrictID}">${d.DistrictName}</option>`;
                });
            }
        })
    ...
    district.addEventListener('change', function() {
        ...
        .then(data => {
            ...
            if (data.status || data.success) {
                data.data.forEach(w => {
                    ward.innerHTML += `<option value="${w.WardName}" data-code="${w.WardCode}">${w.WardName}</option>`;
                });
            }
        })
